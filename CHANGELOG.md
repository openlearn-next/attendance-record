...
- **新增「升级注意：重新上传会生成新的数据表」章节**：在插件中心重新上传 zip 时，宿主会为插件分配新的实例 UUID，私有表随之变为 `plugin_<新UUID>_attendance_logs`，旧实例的历史考勤记录不会自动迁移，教师端明细/汇总会「看起来清空」。README 中补充了升级前备份与手动迁移的 SQL 步骤（`INSERT OR IGNORE ... SELECT`，唯一键保证幂等）。
